...
describe PlanBuilder do
  ...
  let!(:one_private_lesson_one_regular_lessons_paypal_plan_with_weekly_experience) do
    create(:paypal_plan,
  ...
  context 'when select one regular with weekly experience' do
    let(:regular_classes_per_week) { 1 }
  ...
  context 'when select one regular, one private lesson and with week experience' do
    ...
    it 'should build a plan with success' do
      ...
      expect(subject.regular_classes_per_week).to eql(1)
      expect(subject.private_lessons_per_month).to eql(1)
      expect(subject.paypal_plan).to eql(one_private_lesson_one_regular_lessons_paypal_plan_with_weekly_experience)
    end
  end
end
